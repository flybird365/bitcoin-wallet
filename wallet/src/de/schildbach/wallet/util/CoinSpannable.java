...
{
	public CoinSpannable(final MonetaryFormat format, final boolean signed, @Nullable final Monetary monetary)
	{
		super(format(format, signed, monetary));
	}

	public CoinSpannable(final MonetaryFormat format, @Nullable final Monetary monetary)
	{
	...
	private static final Pattern P_PARTS = Pattern.compile("([\\p{Alpha}\\p{Sc}]+\\s)?" //
			+ "([" + Constants.CURRENCY_PLUS_SIGN + Constants.CURRENCY_MINUS_SIGN + "]?(?:\\d*+\\.\\d{0,2}+))" //
			+ "(\\d+)?");
