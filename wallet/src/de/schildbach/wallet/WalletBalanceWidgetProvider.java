			...
			@Nonnull final Coin balance)
	{
		...
		if (data != null)
		{
			if (data.moveToFirst())
			{
				...
				localBalanceStr = new CoinSpannable(localFormat, localBalance).applyMarkup(CoinSpannable.SMALLER_SPAN, new ForegroundColorSpan(
						context.getResources().getColor(R.color.fg_less_significant)), CoinSpannable.SMALLER_SPAN);
			}
			else
			{
